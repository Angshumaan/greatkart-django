        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    # ex_var_list = [4, 6, 3, 5]
                    # a.	Eg: Product_variation = [1,2,3,4,6]
                    # b.	Existing_variation = [4,6,3,5]
                    # c.	We will check if any common number is present in existing_variation
...
def forgotPassword(request):
    if request.method == 'POST':
        email = request.POST['email']
        # filter to get only the emails
        if Account.objects.filter(email=email).exists():
            # get to get the specifice email
...
def change_password(request):
    if request.method == "POST":
        ...
        new_passowrd = request.POST['new_password']
